...
void __global__ jacobi_gpu3_d1(int N, double delta, int kMAX, double *f, double *u_new, double *u_old, double *d0_u_old) {
    int j = blockIdx.x * blockDim.x + threadIdx.x;
    int i = blockIdx.y * blockDim.y + threadIdx.y;
    if (i < (N*0.5-1) && j < (N-1) && i > 0 && j > 0) { // i < N/2
        ...
        u_new[i*N + j] = 0.25 * (d0_u_old[(N/2-1)*N + j] + u_old[(i+1)*N+j] + u_old[i*N + (j-1)] + u_old[i*N + (j+1)] + delta*f[i*N + j]);
    }
}
